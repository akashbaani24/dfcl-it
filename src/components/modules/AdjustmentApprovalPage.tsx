...
export function AdjustmentApprovalPage() {
  ...
  const openView = async (row: any) => {
    try {
      const full = await getOne('adjustments', row.id) as any
      setViewing(full)
    } catch { setViewing(row) }
  }
  ...
  const approve = async () => {
    ...
    setProcessing(true)
    try {
      await action('approve-adjustment', viewing.id, { approver: 'admin' })
  ...
  const reject = async () => {
    ...
    setProcessing(true)
    try {
      await action('reject-adjustment', viewing.id, { approver: 'admin' })
  ...
  return (
    <div>
                  ...
                  {filtered.map((r: any) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="outline" size="sm" onClick={() => openView(r)} className="gap-1">
      ...
      {/* Approval View — full page overlay matching screenshot layout */}
      {viewing && (
            ...
            <div className="grid grid-cols-2 gap-0 border-b-2 border-black">
              <div className="p-3 border-r-2 border-black">
                ...
                <div className="mt-1 flex items-center gap-2 h-10 px-3 border rounded-md bg-slate-50">
                  <span className="text-sm font-medium">{viewing.entity?.name || '—'}</span>
                </div>
              </div>
              <div className="p-3">
                ...
                <div className="mt-1 flex items-center h-10 px-3 border rounded-md bg-slate-50">
                  <span className="text-sm">{new Date(viewing.adjustDate).toLocaleDateString()}</span>
                </div>
              </div>
            </div>
